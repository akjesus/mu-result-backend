...
exports.approveCourses = async (req, res) => {
   ...
   try {
    ...
    if (approved.changedRows > 0) {
        return res.status(200).json({ success: true, code: 200, message: `${approved.changedRows} Results results approved successfully`, courses });
    }
    else {
        return res.status(200).json({ success: true, code: 304, message: 'Warning: No results to aprove for the selected courses', courses });
    }
...
exports.getResultsForApproval = async (req, res) => {
    try {
        const { session, semester } = req.query;
        if (!session || !semester) {
            return res.status(400).json({ success: false, code: 400, message: "Session and Semester are required!" });
        }
        const courses = await Result.getResultsBySessionandSemester(session, semester); 
        return res.status(200).json({ success: true, code: 200, courses });
    } catch (error) {
...
exports.toggleApproval = async (req, res) => {
    ...
    try {  const result = await Result.toggleCourseApproval(session_id, course_id);
        if (!result) {
            return res.status(404).json({ success: false, code: 404, message: 'Course not found' });
        }
        return res.status(200).json({ success: true, code: 200, message: `Result ${approved ? 'unapproved' : 'approved'} successfully` });
    } catch (error) {
