...
exports.login = async (req, res) => {
  ...
  try {
    ...
    user =
      (await Staff.findByUsername(email)) ||
      (await Student.findByUsername(email));
    if (!user) {
      return res
        .status(401)
        .json({
    ...
    if (!user.password) {
      return res.status(500).json({
        success: false,
        code: 500,
        message: "User record is corrupted. No password found.",
      });
    }
...
exports.createAdmin = async (req, res) => {
  try {
    ...
    const newAdminId = await Staff.createStaff(
      first_name,
      last_name,
      email,
      username,
    ...
    if (!newAdminId) {
      return res
        .status(500)
        .json({ success: false, code: 500, message: "Failed to create admin" });
    }
    res.status(201).json({
      success: true,
      code: 201,
  ...
  (req, res, next) => {
    if (!roles.includes(req.user.role)) {
      return res.status(401).json({
        success: false,
        code: 401,
        message: "Unauthorised, Please contact Super Admin!",
      });
    }
    next();
  };
...
exports.resetPassword = async (req, res) => {
  try {
    ...
    if (!user)
      return res
        .status(400)
        .json({ success: false, code: 400, message: "No user in request!" });
