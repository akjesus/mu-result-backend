...
exports.resetPassword = async (req, res) => {
  try {
    ...
    if (!id) {
      return res
        .status(400)
        .json({ success: false, code: 400, message: "ID required!" });
    }
    ...
    return res
      .status(200)
      .json({ success: true, code: 200, message: "Password reset successful" });
  } catch (err) {
    return res
      .status(500)
      .json({ success: false, code: 500, message: err.message });
...
exports.blockUser = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      return res
        .status(404)
        .json({ success: false, code: 500, message: "User not found" });
    }
...
exports.bulkUploadStudents = async (req, res) => {
  try {
    ...
    const fileExtension = studentsFile.originalname
      .split(".")
      .pop()
      .toLowerCase();
...
exports.bulkDownloadStudents = async (req, res) => {
  try {
    const [students] = await Student.execute(`
      SELECT mat_no as MatricNumber, concat(last_name,' ', first_name, ' ' other_names) as Fullname, 
...
exports.updateProfilePicture = async (req, res) => {
  try {
    ...
    if (!imageUrl) {
      return res
        .status(400)
        .json({ success: false, code: 400, message: "Image URL is required" });
    }
    await Student.execute("UPDATE students SET photo = ? WHERE id = ?", [
      imageUrl,
      userId,
    ]);
    return res.status(200).json({
...
exports.getStudentsByDepartment = async (req, res) => {
  ...
  if (!departmentId) {
    return res
      .status(400)
      .json({ success: false, code: 400, message: "Department required" });
  }
  try {
    let query = `
      SELECT students.id as id, concat(first_name, ' ', other_names, ' ', last_name) as name,
...
exports.resetAllPasswords = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      code: 200,
      message: `Passwords reset successful for ${affectedRows} students`,
    });
  } catch (error) {
