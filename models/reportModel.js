...
class Report {
  static async getStudentResultsbyGrade(grade) {
    const [rows] = await db.query("SELECT * FROM results WHERE grade = ?", [
      grade,
    ]);
    return rows;
  }
  ...
  static async getGradeDistributionByDepartment(
    ...
    const [rows] = await db.query(
      `
        ...
        GROUP BY r.grade
        ORDER BY FIELD(r.grade, 'A', 'B', 'C', 'D', 'E', 'F')`,
      [departmentId, semesterId, sessionId],
    );
    return rows;
  }
  ...
  static async getGradeDistributionByFaculty(facultyId, semesterId, sessionId) {
    const [rows] = await db.query(
      `
        ...
        GROUP BY r.grade
        ORDER BY FIELD(r.grade, 'A', 'B', 'C', 'D', 'E', 'F')`,
      [facultyId, semesterId, sessionId],
    );
    return rows;
  }
